...
When enough info is collected, generate a LONG FINAL PRESCRIPTION.
...
"""
    msgs=[{"role":"system","content":prompt}]+st.session_state.chat
    ...
    if st.session_state.show_patient:
        with left:
            ...
            for l,v in [
                ...
                ("Weight",patient[5]),("Allergy",patient[6]),
                ("Language",patient[7])
            ]:
                st.write(f"**{l}:** {v}")
    ...
    with right:
        ...
        if st.session_state.final_rx:
            ...
            with open(pdf,"rb") as f:
                st.download_button("⬇ Download Prescription PDF",f,
